    ...
    for n_heads in args.n_heads:
        ...
        for concept in args.concepts:
            ...
            for steer_weight in weights:
                ...
                # Save results
                pickle.dump(layer_evals, open(os.path.join(OUTPUT_DIR, f"layer_evals_x{steer_weight}_n{n_heads}.pkl"), 'wb'))
                np.save(os.path.join(OUTPUT_DIR, f"delta_probs_x{steer_weight}_n{n_heads}.npy"), np.stack(delta_probs))
                if intervention_type == 'ambiguous':
                    np.save(os.path.join(OUTPUT_DIR, f"delta_probs_lang_x{steer_weight}_n{n_heads}.npy"), np.stack(delta_probs_lang))
                    np.save(os.path.join(OUTPUT_DIR, f"kl_divs_x{steer_weight}_n{n_heads}.npy"), kl_divs.numpy())